...
class Client(object):
    def __init__(self, key, locale=None, region="ap", route="asia", reload=True):
        self.key = key
        self.route = route
        self.locale = locale
        self.region = region
...
# original
#     def get_matchlist(self, size: int=100, page: int=0, name: str="") -> MatchlistDTO:
#         heads = self.build_header({"X-Riot-Token": self.key})
#         params = {"locale": self.locale, "size": size, "startIndex": size * page}
#
#         get_puuid = self.get_user_by_name(name)
#         puuid = get_puuid.puuid
#
#         url = self.build_url(code=self.route, endpoint="match-history")
#         url = url.format(puuid=puuid)
#
#         r = self.fetch(url, headers=heads)
#         r.raise_for_status()
#
#         return matchlistDTO(r.json())
#
#     def get_match(self, matchId: str) -> MatchDTO:
#         heads = self.build_header({"X-Riot-Token": self.key})
#         Id = matchId
#
#         url = self.build_url(code=self.route, endpoint="match")
#         url = url.format(matchId=Id)
#
#         r = self.fetch(url, headers=heads)
#         r.raise_for_status()
#
#         return MatchDTO(r.json())
